...
def lava_lamp_gradient(width: int, height: int, colors: list[str], positions: list[float] | None = None) -> Image.Image:
    """Generate lava lamp gradient using metaball inverse-square blending."""
    rng = np.random.default_rng(42)
    stops = [hex_to_rgb(c) for c in colors]
    num_blobs = len(stops)

    bx = [rng.uniform(0.1, 0.9) for _ in range(num_blobs)]
    by = [rng.uniform(0.1, 0.9) for _ in range(num_blobs)]
    ...
    for y in range(height):
        for x in range(width):
            nx = x / max(width - 1, 1)
            ny = y / max(height - 1, 1)
            r_acc = g_acc = b_acc = w_acc = 0.0
            for i, (cr, cg, cb) in enumerate(stops):
                dx = nx - bx[i]
                dy = ny - by[i]
                dist_sq = max(dx*dx + dy*dy, 1e-6)
                ...
                w_acc += influence
            pixels[x, y] = (
                int(min(255, max(0, r_acc / w_acc))),
                int(min(255, max(0, g_acc / w_acc))),
                int(min(255, max(0, b_acc / w_acc))),
            )
    return img
...
if NUMPY_AVAILABLE:
    ...
    def lava_lamp_gradient_fast(width: int, height: int, colors: list[str], positions: list[float] | None = None) -> Image.Image:
        """Fast lava lamp gradient using NumPy metaball inverse-square blending."""
        rng = np.random.default_rng(42)
        stops = np.array([hex_to_rgb(c) for c in colors], dtype=np.float32)
        num_blobs = len(stops)
        ...
        r_acc = np.zeros((height, width), dtype=np.float32)
        g_acc = np.zeros((height, width), dtype=np.float32)
        b_acc = np.zeros((height, width), dtype=np.float32)
        w_acc = np.zeros((height, width), dtype=np.float32)

        for i in range(num_blobs):
            dx = gx - bx[i]
            dy = gy - by[i]
            dist_sq = np.maximum(dx*dx + dy*dy, 1e-6)
            influence = 1.0 / dist_sq
            r_acc += influence * stops[i][0]
            g_acc += influence * stops[i][1]
            b_acc += influence * stops[i][2]
            w_acc += influence

        w_acc = np.maximum(w_acc, 1e-10)
        img_array = np.stack([
            np.clip(r_acc / w_acc, 0, 255).astype(np.uint8),
            np.clip(g_acc / w_acc, 0, 255).astype(np.uint8),
            np.clip(b_acc / w_acc, 0, 255).astype(np.uint8),
        ], axis=2)
        return Image.fromarray(img_array, mode='RGB')
    ...
    def to_image(self) -> Image.Image:
        ...
        if NUMPY_AVAILABLE:
            match self.direction:
                ...
                case "Lava Lamp":
                    return lava_lamp_gradient_fast(self.width, self.height, self.palette.colors, get_palette_positions(self.palette))
                case _:
            ...
            match self.direction:
                ...
                case "Lava Lamp":
                    return lava_lamp_gradient(self.width, self.height, self.palette.colors, get_palette_positions(self.palette))
                case _:
