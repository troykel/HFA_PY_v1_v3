...
# Replaces QColorDialog throughout the app for low-vision accessibility.
# Smaller wheel (260px) + large fonts so text is always readable.
# --------------------------------------------------------------------------------------

class ColorWheelWidget(QWidget):
    """Draws an HSV wheel: outer hue ring + inner saturation/value triangle."""
    from PySide6.QtCore import Signal as _Signal
    colorChanged = _Signal(QColor)
    RING = 24  # hue ring thickness
    ...
    def _tri_vertices(self):
        """A=pure hue top, B=black bottom-left, C=white bottom-right."""
        cx, cy = self._cx(), self._cy()
        r = self._radius() - self.RING - 8
        a = math.radians(self._hue - 90)
        b = a + math.radians(120)
        c = a - math.radians(120)
        return (
            (cx + r * math.cos(a), cy + r * math.sin(a)),
            (cx + r * math.cos(b), cy + r * math.sin(b)),
            (cx + r * math.cos(c), cy + r * math.sin(c)),
        )

    def _sv_to_point(self, s, v):
        """Convert sat/val to pixel position inside triangle."""
        A, B, C = self._tri_vertices()
        x = A[0] * v + B[0] * (1 - v) * s + C[0] * (1 - v) * (1 - s)
        y = A[1] * v + B[1] * (1 - v) * s + C[1] * (1 - v) * (1 - s)
        return x, y

    def _point_to_sv(self, px, py):
        """Convert pixel position to sat/val using barycentric coords."""
        A, B, C = self._tri_vertices()
        denom = (B[1] - C[1]) * (A[0] - C[0]) + (C[0] - B[0]) * (A[1] - C[1])
        if abs(denom) < 1e-9:
            return self._sat, self._val
        v = ((B[1] - C[1]) * (px - C[0]) + (C[0] - B[0]) * (py - C[1])) / denom
        s_bary = ((C[1] - A[1]) * (px - C[0]) + (A[0] - C[0]) * (py - C[1])) / denom
        v = max(0.0, min(1.0, v))
        s_bary = max(0.0, min(1.0, s_bary))
        # Normalise so coords sum to <= 1
        total = v + s_bary
        if total > 1.0:
            v /= total
            s_bary /= total
        return max(0.0, min(1.0, s_bary)), max(0.0, min(1.0, v))
    ...
    def paintEvent(self, event):
        ...
        for deg in range(360):
            rad_s = math.radians(deg - 90)
            rad_e = math.radians(deg - 89)
            hue_color = QColor.fromHsvF(deg / 360.0, 1.0, 1.0)
            painter.setPen(hue_color)
            # Draw thick arc segment as a small filled polygon
            outer = R
            inner = R - ring
            pts = [
                (cx + outer * math.cos(rad_s), cy + outer * math.sin(rad_s)),
                (cx + outer * math.cos(rad_e), cy + outer * math.sin(rad_e)),
                (cx + inner * math.cos(rad_e), cy + inner * math.sin(rad_e)),
                (cx + inner * math.cos(rad_s), cy + inner * math.sin(rad_s)),
            ]
        ...
        from PySide6.QtGui import QPolygonF
        from PySide6.QtCore import QPointF
        tri = QPolygonF([QPointF(A[0], A[1]), QPointF(B[0], B[1]), QPointF(C[0], C[1])])

        # Draw triangle using a gradient approximation:
        # Layer 1: white->hue gradient
        grad1 = QLinearGradient(C[0], C[1], A[0], A[1])
        grad1.setColorAt(0, QColor(255, 255, 255))
        grad1.setColorAt(1, hue_color)
        painter.setBrush(grad1)
        painter.setPen(Qt.NoPen)
        painter.drawPolygon(tri)

        # Layer 2: transparent->black gradient (top to bottom)
        mid_ab_x = (A[0] + B[0]) / 2
        mid_ab_y = (A[1] + B[1]) / 2
        grad2 = QLinearGradient(A[0], A[1], mid_ab_x, mid_ab_y)
        grad2.setColorAt(0, QColor(0, 0, 0, 0))
        grad2.setColorAt(1, QColor(0, 0, 0, 255))
        painter.setBrush(grad2)
        painter.drawPolygon(tri)

        # - Hue ring selector dot -
        angle = math.radians(self._hue - 90)
        dot_r = R - ring / 2
        ...
        # - Triangle selector dot -
        tx, ty = self._sv_to_point(self._sat, self._val)
        painter.setPen(Qt.white)
    ...
    def mousePressEvent(self, event):
        self._handle_mouse(event.position().x(), event.position().y(), press=True)

    def mouseMoveEvent(self, event):
        self._handle_mouse(event.position().x(), event.position().y(), press=False)

    def mouseReleaseEvent(self, event):
        self._drag_ring = False
        self._drag_tri = False

    def _handle_mouse(self, mx, my, press):
        cx, cy = self._cx(), self._cy()
        R = self._radius()
        dist = math.sqrt((mx - cx) ** 2 + (my - cy) ** 2)

        if press:
            self._drag_ring = (R - self.RING) <= dist <= R
            self._drag_tri = dist < (R - self.RING - 4)

        if self._drag_ring:
            self._hue = (math.degrees(math.atan2(my - cy, mx - cx)) + 90) % 360
            self.update()
            self.colorChanged.emit(self.color())

        elif self._drag_tri:
            self._sat, self._val = self._point_to_sv(mx, my)
            self.update()
            self.colorChanged.emit(self.color())


class ColorWheelDialog(QDialog):
    """Accessible color picker dialog with HSV wheel and large fonts."""

    _FONT_PT = 20  # large font for low-vision accessibility
    ...
    def _build_ui(self):
        ...
        # - Top: wheel + preview side by side -
        top_row = QHBoxLayout()

        self.wheel = ColorWheelWidget()
        self.wheel.setColor(self._color)
        # Always open at full brightness/saturation so users start in the vivid zone
        self.wheel._sat = 1.0
        self.wheel._val = 1.0
        ...
        self.preview.setStyleSheet(
            f"background-color: {self._color.name()}; border: 3px solid white; border-radius: 8px;"
        )
        right_col.addWidget(self.preview)

        # RGB readouts
        font = QFont("Arial", self._FONT_PT, QFont.Bold)
        self.r_label = QLabel()
        self.g_label = QLabel()
        self.b_label = QLabel()
        for lbl in (self.r_label, self.g_label, self.b_label):
            lbl.setFont(font)
    ...
    def _refresh_readouts(self):
        ...
        self.preview.setStyleSheet(
            f"background-color: {c.name()}; border: 3px solid white; border-radius: 8px;"
        )
...
def pick_color(initial: QColor = None, parent=None, title: str = "Choose Color") -> QColor:
    ...
    Returns the chosen QColor, or an invalid QColor if cancelled.
    """
...
class MainWindow(QMainWindow):
    ...
    def _build_left_panel(self, splitter: QSplitter) -> None:
        ...
        self.direction_combo.currentTextChanged.connect(lambda _: self._update_status_line(None))
        ...
        self.width_spin.textChanged.connect(lambda _: self._update_status_line(None))
        ...
        self.height_spin.textChanged.connect(lambda _: self._update_status_line(None))
...
if __name__ == "__main__":
    ...
    win.show()
    sys.exit(app.exec())  # HFA_2025.py - Refactored
# -*- coding: utf-8 -*-
"""
Huemanity for ALL (PySide6) - 2026 Edition
...
class MainWindow(QMainWindow):
    ...
    def on_random_clicked(self) -> None:
        ...
        # Select the random palette
        self._apply_filters(select_name=random_palette_name)

        # Also randomize direction and dimensions
        self.direction_combo.setCurrentText(random.choice([
            "Conical (Sweep)",
            "Diagonal (TL to BR)",
            "Diamond",
            "Horizontal",
        ...
        # AUTO-GENERATE: Automatically create the preview!
        self.on_generate_clicked()
        self.width_spin.setValue(random.randint(640, 1640))
        self.height_spin.setValue(random.randint(360, 1080))
